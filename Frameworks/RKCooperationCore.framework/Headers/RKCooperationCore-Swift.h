...
///
- (void)cancelWithChannelId:(NSString * _Nonnull)channelId onSuccess:(void (^ _Nullable)(id _Nullable))onSuccess onfailed:(void (^ _Nullable)(NSError * _Nullable))onfailed SWIFT_DEPRECATED_MSG("2.2.1 弃用");
/// 取消邀请
/// 对方将收到 RKIncomingCallListener.onCallCanceled 回调
/// <ul>
///   <li>
///     Parameters
///   </li>
...
///
- (void)onInviteResultWithChannelId:(NSString * _Nonnull)channelId userId:(NSString * _Nonnull)userId result:(BOOL)result reason:(enum RKCooperationCode)reason SWIFT_DEPRECATED_MSG("2.0.0 弃用");
/// 呼叫或加入失败了
/// \param channelId 频道ID 
///
/// \param reason 失败的原因 
...
///
- (void)onRecordingStateChanged:(RKIRecordingStateModel * _Nonnull)recordingStateData;
@end
...
- (void)stopServerRecordingWithChannelId:(NSString * _Nonnull)channelId save:(BOOL)save;
/// 获取频道录制的所有文件
- (void)getServerRecordingFilesWithChannelId:(NSString * _Nonnull)channelId;
/// 删除频道内录制的视频文件
- (void)deleteServerRecordingFilesWithChannelId:(NSString * _Nonnull)channelId;
@end
...
/// 最大延迟，默认根据后台配置
@property (nonatomic) int32_t maxDelay;
/// 默认的音视频订阅状态，调用方在创建或加入频道的时候可以指定，默认是both
@property (nonatomic) enum SubscribeMediaType defaultSubscribeMediaType;
@property (nonatomic) BOOL defaultStreamType;
- (nonnull instancetype)init OBJC_DESIGNATED_INITIALIZER;
@end
...
@property (nonatomic, readonly) enum SubscribeMediaType mediaSubscribeState;
@property (nonatomic, readonly) BOOL isHighStream;
- (void)subscribeStateChange:(enum SubscribeMediaType)state onSuccess:(void (^ _Nullable)(id _Nullable))onSuccess onFailed:(void (^ _Nullable)(NSError * _Nullable))onFailed;
- (void)switchStreamWithIsHighStream:(BOOL)isHighStream onSuccess:(void (^ _Nullable)(id _Nullable))onSuccess onFailed:(void (^ _Nullable)(NSError * _Nullable))onFailed;
- (nonnull instancetype)init OBJC_DESIGNATED_INITIALIZER;
@end
...
/// \param password 密码
///
/// \param onSuccess 成功回调  [String: Any] token 及用户信息
///
/// \param onFailed 失败回调
///
...
/// sdk登录 token登录
- (void)loginWith:(NSString * _Nonnull)token userInfo:(RKUser * _Nonnull)userInfo SWIFT_DEPRECATED_MSG("2.2.0 弃用, Use login(with appId:) instead.");
/// 更新token，外部更新token，在token失效之前更新
- (void)updateToken:(NSString * _Nonnull)token SWIFT_DEPRECATED_MSG("2.2.0 弃用");
/// sdk登出
- (void)logout;
/// sdk销毁
- (void)destroy;
...
/// 输出到指定音频设备
/// \param device @RKIAudioDevice 
///
+ (void)selectAudioWithDevice:(enum RKIAudioDevice)device;
/// 获取所有可输出设备
/// 返回 [RKIAudioDevice]
...
- (void)onCameraUpdate;
/// 音频输出变更
- (void)onAudioOutputTypeChangeWithAudioType:(enum RKIAudioDevice)audioType;
/// 音频输出状态发生改变
...
/// 当前音频输出设备
typedef SWIFT_ENUM(int32_t, RKIAudioDevice, open) {
  RKIAudioDeviceSpeakerPhone = 1,
  RKIAudioDeviceEarpiece = 2,
  RKIAudioDeviceWiredHeadset = 3,
  RKIAudioDeviceBluetooth = 4,
};
...
@interface RKIRecordingStateModel : NSObject
...
typedef SWIFT_ENUM(int32_t, RKIStreamType, open) {
/// 大流
  RKIStreamTypeHigh = 0,
/// 小流
  RKIStreamTypeLow = 1,
...
///   </li>
///   <li>
///     channelParam 频道参数 @ RKChannelParam
///   </li>
/// </ul>
...
/// 上传中
  RKRecordStateRecording = 0,
/// 上传中
  RKRecordStateUploading = 1,
/// 录制完成
  RKRecordStateDone = 2,
/// 录制错误
  RKRecordStateError = 3,
};
...
typedef SWIFT_ENUM(int32_t, RKRotation, open) {
...
/// 订阅音视频状态
typedef SWIFT_ENUM(NSInteger, SubscribeMediaType, open) {
/// 都订阅
  SubscribeMediaTypeBoth = 0,
/// 仅仅音频订阅
  SubscribeMediaTypeAudio = 1,
/// 仅视频订阅
  SubscribeMediaTypeVideo = 2,
/// 都不订阅
  SubscribeMediaTypeNone = 3,
};
